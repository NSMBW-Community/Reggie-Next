...
def LoadTileset(idx, name, reload_=False):
    ...
    if name == "Pa1_toride" and False:
        print("Loading toride")
        data = []

        for i, obj in enumerate(defs):
            if not obj: continue

            # Get the tiles this object refers to
            solid = []
            skip_obj = False
            ...
            for row in obj.rows:
                ...
                for tile_data in row:
                    if len(tile_data) == 1:
                        tile_id = tile_data[0]
                    elif len(tile_data) == 3:
                        tile_id = tile_data[1]
                    else:
                        print("???")

                    if tile_id == 256:  # That weird pink thing
                        skip_obj = True
                        break

                    tile = globals_.Tiles[tile_id]
                    CD = tile.collData

                    if (CD[3] & 32 and CD[7] != 10) or CD[3] & 64 or CD[5] or CD[2] & 16:  # slope / other weird thing
                        skip_obj = True
                        break

                    if not (CD[3] & 1 or (CD[3] & 32 and CD[7] == 10)):
                        # Not solid
                        block_type = 0
                    else:
                        img = tile.main.toImage()
                        tot_r = tot_g = tot_b = 0
                        ...
                        block_type = tot_r << 16 | tot_g << 8 | tot_b
                        if block_type < 3_000_000_000:
                            block_type = 2  # green / edge
                        else:
                            block_type = 1  # normal / center

                    row_solid.append(block_type)
